...
subroutine HydrateAccumulation(hyd_auxvar,global_auxvar,material_auxvar, &
  ...
  PetscInt :: wat_comp_id, air_comp_id, energy_id, hyd_id, ice_id
...
subroutine HydrateFlux(hyd_auxvar_up,global_auxvar_up, &
  ...
  if (dabs(dist_gravity) > 0.d0) then
    ...
    if (upwind) then
      ...
      gas_sat = min(hyd_auxvar_up%sat(gid),material_auxvar_up%srg)
      liq_sat = min(hyd_auxvar_up%sat(lid),material_auxvar_up%srl)
      ...
      energy_flux = q * (hyd_auxvar_up%den(lid) * hyd_auxvar_up%H(lid) * liq_sat + &
                hyd_auxvar_up%den(gid) * hyd_auxvar_up%H(gid) * gas_sat + &
      ...
      gas_sat = min(hyd_auxvar_dn%sat(gid),material_auxvar_dn%srg)
      liq_sat = min(hyd_auxvar_dn%sat(lid),material_auxvar_dn%srl)
      ...
      energy_flux = q * (hyd_auxvar_dn%den(lid) * hyd_auxvar_dn%H(lid) * liq_sat + &
    ...
    case(NEUMANN_BC)
      ...
      if (dabs(auxvars(idof)) > floweps) then
        ...
        if (v_darcy(iphase) > 0.d0) then 
          ...
          xmol(wat_comp_id) = hyd_auxvar_dn%xmol(wat_comp_id,iphase)
          xmol(air_comp_id) = hyd_auxvar_dn%xmol(air_comp_id,iphase)
        endif 
      endif
    case default
      option%io_buffer = &
