  ...
  if (option%myrank == option%io_rank) then
    ...
    do irank = 0, option%mycommsize-1
      ...
      do icell = 1, num_to_read
        ...
        call input%ReadInt(option,temp_int)
        call input%ErrorMsg(option,'num faces',hint)
        ...
        call input%ReadInt(option,temp_int)
        call input%ErrorMsg(option,'num vertices',hint)
        ...
        call input%ReadDouble(option,temp_real_array(6,icell))
        call input%ErrorMsg(option,'cell z coordinate',hint)

        call input%ReadDouble(option,temp_real_array(7,icell))
  ...
  if (option%myrank == option%io_rank) then
    ...
    do irank = 0, option%mycommsize-1
      ...
      do iface = 1, num_to_read
        ...
        call input%ReadInt(option,temp_int)
        call input%ErrorMsg(option,'face id',hint)
        ...
        call input%ReadInt(option,temp_int)
        call input%ErrorMsg(option,'number of vertices',hint)
        ...
        do ivert = 1, int(temp_real_array(3,iface))
          call input%ReadInt(option,temp_int)
          call input%ErrorMsg(option,'face - vertex id',hint)
          temp_real_array(3+ivert,iface) = dble(temp_int)
        enddo

        call input%ReadDouble(option,temp_real_array(4+max_nvert_per_cell,iface))
        call input%ErrorMsg(option,'face x coordinate',hint)
        call input%ReadDouble(option,temp_real_array(5+max_nvert_per_cell,iface))
        call input%ErrorMsg(option,'face y coordinate',hint)
        call input%ReadDouble(option,temp_real_array(6+max_nvert_per_cell,iface))
        call input%ErrorMsg(option,'face z coordinate',hint)
        call input%ReadDouble(option,temp_real_array(7+max_nvert_per_cell,iface))
  ...
  if (option%myrank == option%io_rank) then
    ...
    do irank = 0, option%mycommsize-1
      ...
      do ivert = 1, num_to_read
        call InputReadPflotranString(input,option)
        call input%ReadStringErrorMsg(option,hint)
        call input%ReadDouble(option,temp_real_array(1,ivert))
        call input%ErrorMsg(option,'vertex x coordinate',hint)
        call input%ReadDouble(option,temp_real_array(2,ivert))
        call input%ErrorMsg(option,'vertex y coordinate',hint)
        call input%ReadDouble(option,temp_real_array(3,ivert))
        call input%ErrorMsg(option,'vertex z coordinate',hint)
      enddo
...
#include "petsc/finclude/petscdm.h"
  ...
  if (index_format_flag /= 0 .and. index_format_flag /= 1) then
    call option%PrintErrMsg('Min. vertex id is neither 0 nor 1. Check input mesh.')
  endif
...
#if UGRID_DEBUG
  call option%PrintMsg('Dual matrix')
#endif
...
#if UGRID_DEBUG
  call option%PrintMsg('Setting cell types')
#endif
